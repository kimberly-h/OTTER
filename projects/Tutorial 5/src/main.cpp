...
bool initGLFW() {
	...
	window = glfwCreateWindow(800, 800, "Assignment 1: Object Loader | Kimberly Hansuwan 100752265", nullptr, nullptr);
...
int main() {
	...
	vao2->AddVertexBuffer(interleaved_vbo, VertexPosCol::V_DECL); //V_Decl is a list of buffers to add to the VAO 
	...
	VertexArrayObject::sptr vao5 = OBJLoader::FileLoader("monkey.obj"); //loading monkey
	...
	transform4 = glm::translate(transform4, glm::vec3(0.0f, -1.5f, 0.0f)); //so that the monkiees aren't inside each other
	...
	while (!glfwWindowShouldClose(window)) {
		...
		if (glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS) {
			...
			//camera->SetPosition(camera->GetPosition() + glm::vec3(type soemthing in here becasue it'll show up));
		}
		...
		if (glfwGetKey(window, GLFW_KEY_SPACE) == GLFW_PRESS) 
		{
			if (!beingPressed)
			{
				if (switchMode)
				{
					camera->setPerspective(switchMode);
					switchMode = false;
				}
		...
		transform5 = glm::rotate(transform5, glm::radians(rot), glm::vec3(0, 3, 0));
