	...
	const glm::vec3& tangent, const glm::vec2& scale, const glm::vec4& col)
{
	...
	mesh.AddIndexTri(p1, p3, p2); //feeds the points to make it into a triangle (adds 3 indexes to make 1 tri)
...
void MeshFactory::AddCube(MeshBuilder<VertexPosNormTexCol>& mesh, const glm::mat4& transform, const glm::vec4& col) {
	...
	glm::vec2 uvs[] = { //Different sizes of arrays for different attributes
		glm::vec2(0.0f, 0.0f), // 0
		glm::vec2(0.0f, 1.0f), // 1
		glm::vec2(1.0f, 1.0f), // 2
		glm::vec2(1.0f, 0.0f), // 3
	...
	// Bottom
	indices[0] = mesh.AddVertex(positions[0], normals[4], uvs[0], col); // Different indecies, create a new vertex for each indecies
