...
//Uniform for mixing
uniform float u_TexturedMix; //between 0 and 1
...
void main() {
	...
	//lerp function
	vec4 textureLerp = (1.0 - u_TexturedMix) * textureColor + u_TexturedMix * textureColor2;
	vec3 result = (
		(u_AmbientCol * u_AmbientStrength) + // global ambient light
		(ambient + diffuse + specular) * attenuation // light factors from our single light
		) * inColor * textureLerp.rgb; // Object color

	frag_color = vec4(result, textureLerp.a);
}